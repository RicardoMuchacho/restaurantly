...
const RestaurantCard = ({restaurant}:Props) => {
  ...
  return(
      <div style={{width: "280px", height: "280px"}} className="card p-0">
        <div className="">
          <img width={279} height={160} className="card-image object-fit-cover" src={photoUrl} alt={restaurant.name} />
        </div>
        <div className="px-3 py-2">
          ...
          <div className="d-flex flex-row justify-content-between py-1">
            <div className="d-flex flex-column gap-1">
              <span className="m-0 d-inline" style={{color: priceLevelMapping[restaurant.price]?.color}}>Price: {priceLevelMapping[restaurant.price]?.text || "$$"}</span>
